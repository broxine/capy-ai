...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:  # noqa: N802
        if self.path == "/":
            ...
            self.end_headers()
            self.wfile.write(
                b"<html><title>Review Target</title><body><form action='/login'><input name='email'></form><script>window.location.hash='callback';window.postMessage('x','*');</script>oauth wallet permit callback</body></html>"
            )
            return
...
def test_review_browser_validation_generates_candidates(tmp_path: Path) -> None:
    ...
    try:
            ...
            json.dumps(
                {
                    ...
                    "reports": [{"source": str(report), "title": "Review Report", "kind": "report"}],
        ...
        assert payload["program_slug"] == "review-demo"
        assert payload["candidates"]
        categories = {item["category"] for item in payload["candidates"]}
        assert "callback-parameter" in categories or "wallet-signature" in categories or "login-surface" in categories
    finally:
        server.shutdown()
        server.server_close()
