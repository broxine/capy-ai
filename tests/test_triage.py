...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:  # noqa: N802
        if self.path == "/":
            ...
            self.end_headers()
            self.wfile.write(
                b"<html><title>Triage Target</title><body><form action='/login'><input name='email'></form><script>window.postMessage('x','*');</script>oauth callback wallet permit</body></html>"
            )
            return
...
def test_triage_review_candidates_ranks_and_dedupes(tmp_path: Path) -> None:
    ...
    try:
            ...
            json.dumps(
                {
                    ...
                    "reports": [{"source": str(report), "title": "Triage Report", "kind": "report"}],
        ...
        assert payload["program_slug"] == "triage-demo"
        assert payload["findings"]
        assert payload["findings"][0]["severity"] in {"medium", "high"}
