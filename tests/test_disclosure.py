...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:  # noqa: N802
        if self.path == "/":
            ...
            self.end_headers()
            self.wfile.write(
                b"<html><title>Disclosure Target</title><body><form action='/login'><input name='email'></form><script>window.postMessage('x','*');</script>oauth callback wallet permit</body></html>"
            )
            return
...
def test_generate_disclosure_drafts_writes_markdown(tmp_path: Path) -> None:
    ...
    try:
            ...
            json.dumps(
                {
                    ...
                    "reports": [{"source": str(report), "title": "Disclosure Report", "kind": "report"}],
        ...
        disclosure_path = generate_disclosure_drafts(
            repo_root=tmp_path,
        ...
        assert payload["program_slug"] == "disclosure-demo"
        assert payload["drafts"]
        draft_path = tmp_path / payload["drafts"][0]["report_markdown_path"]
        assert "Impact hypothesis" in draft_path.read_text(encoding="utf-8")
    finally:
        server.shutdown()
        server.server_close()
