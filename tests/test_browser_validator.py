...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:  # noqa: N802
        if self.path == "/":
            ...
            self.end_headers()
            self.wfile.write(
                b"<html><title>Browser Target</title><body><a href='/login'>Login</a><script>window.demo=1</script></body></html>"
            )
            return
...
def test_run_browser_validation_writes_bundle(tmp_path: Path) -> None:
    ...
    try:
            ...
            json.dumps(
                {
                    ...
                    "reports": [{"source": str(report), "title": "Browser Report", "kind": "report"}],
        ...
        bundle_path = build_validation_bundle(tmp_path, run_path, max_tasks=1)
        ...
        browser_bundle_path = run_browser_validation(
            repo_root=tmp_path,
        ...
        payload = json.loads(browser_bundle_path.read_text(encoding="utf-8"))
        assert payload["program_slug"] == "browser-demo"
        assert payload["executions"]
        assert payload["executions"][0]["screenshot_artifact"].endswith(".png")
        assert payload["executions"][0]["dom_artifact"].endswith(".html")
        assert (tmp_path / payload["executions"][0]["dom_artifact"]).exists()
        assert (tmp_path / payload["executions"][0]["screenshot_artifact"]).exists()
    finally:
        server.shutdown()
        server.server_close()
